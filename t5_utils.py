...
def initialize_model(args):
    ...
    '''
    if args.finetune:
        print("fetching pretrained T5-small model...")
        model = T5ForConditionalGeneration.from_pretrained('google-t5/t5-small')
    else:
        print("creating T5-small model from scratch...")
        config = T5Config.from_pretrained('google-t5/t5-small')
        model = T5ForConditionalGeneration(config) # type: ignore
...
def load_model_from_checkpoint(args, best):
    ...
    print(f"Loading model from {checkpoint_path}") # type: ignore
